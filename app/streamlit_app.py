...
from src.features.document_validation import (extract_text,extract_land_acres,extract_income,extract_vehicle_price,validate_customer_name,validate_land,validate_income,validate_quotation)
...
if st.session_state["current_tab"] == "Customer Info":
    ...
    with col2:
         ...
         # Calculate age
         today = date.today()
         age_str = (today.year- dob.year- ((today.month, today.day) < (dob.month, dob.day)))
...
# Navigation buttons
    ...
    no_land_document = st.checkbox("Customer does not own agricultural land")
    no_income_document = st.checkbox("No documented income available")

    land_document = None
    income_document = None
    ...
    mandatory_missing = (identity_document is None or vehicle_quotation is None)
    ...
    with col_next:
        if st.button( "Next ➡️",key="validate_documents"):
            ...
            st.session_state["no_land_document"] = no_land_document
            st.session_state["no_income_document"] = no_income_document
            st.rerun()
     ...
     features = None
     customer_name = st.session_state.get("customer_name", "")
     age = st.session_state.get("age_str", "")
     ...
     no_land_document = st.session_state.get("no_land_document",False)
     no_income_document = st.session_state.get("no_income_document",False)
     ...
     if manufacturer in [None, "Select..."] or vehicle_model in [None, "Select..."]:
         ...
         def compute_underwriting( age,cibil_score,state, district, crop, land_acres, other_income, obligation,
             ...
             # Land document
                 if no_land_document:
                     land_result = {"status": "NOT APPLICABLE"}
                 else:
                     land_text = extract_text(land_document)
                     land_result = validate_land(land_acres,land_text)
             
              # Income document
                 if no_income_document:
                     income_result = {"status": "NOT APPLICABLE"}
                 else:
              ...
              # Vehicle quotation
                 ...
                 land_ok = land_result["status"] in ["✅ MATCH","NOT APPLICABLE"]
                 income_ok = income_result["status"] in ["✅ MATCH","NOT APPLICABLE"]

                 all_documents_match = (
                    name_result["status"] == "✅ MATCH"
                    and land_ok
                    and income_ok
                    and quotation_result["status"] == "✅ MATCH")
                 ...
                 if not all_documents_match:
                             ...
                             if income_result["status"] == "❌ MISMATCH":
                                 ...
                                 f"₹{income_result['entered']:} entered vs "
                                 f"₹{income_result['document']:} in document")
  
                             if quotation_result["status"] == "❌ MISMATCH":
                                 st.error(f"❌ MISMATCH: "
                                 f"₹{quotation_result['entered']:} as reported by Manufacturer vs "
                                 f"₹{quotation_result['document']:}in quotation document")
         ...
         result = compute_underwriting(age, cibil_score,state, district, crop, land_acres, other_income, obligation,loan_amount, loan_tenure, manufacturer, vehicle_model, existing_customer)

         if result is None:
             if st.button( "Restart Login ➡️",key="restart"):
                        st.session_state["current_tab"] = "Customer Info"
                        st.rerun()
